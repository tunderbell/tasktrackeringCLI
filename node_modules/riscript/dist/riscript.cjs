...
var RiScriptVisitor = class extends BaseVisitor {
  ...
  assign(ctx, opts) {
    ...
    const isSilent = _optionalChain([opts, 'optionalAccess', _ => _.silent]);
  ...
  symbol(ctx, opts) {
    ...
    const isSilent = _optionalChain([opts, 'optionalAccess', _2 => _2.silent]);
    const original = this.nodeText;
    ...
    if (typeof result === "undefined") {
      this.print("/symbol", sym + " -> '" + original + "' ctx=" + this.lookupsToString(), "[deferred]", _optionalChain([opts, 'optionalAccess', _3 => _3.silent]) ? "{silent}" : "");
      return original;
    }
    let info = _optionalChain([opts, 'optionalAccess', _4 => _4.trace]) ? `${original.replace(/\(\)$/, "")} -> ${formatAny(result)}` + (_optionalChain([opts, 'optionalAccess', _5 => _5.silent]) ? " {silent}" : "") : null;
  ...
  choice(ctx, opts) {
    ...
    let decision = "accept";
    if (_optionalChain([opts, 'optionalAccess', _6 => _6.forceReject])) {
      decision = "reject";
    } else {
      let gateCtx = _optionalChain([ctx, 'optionalAccess', _7 => _7.gate, 'optionalAccess', _8 => _8[0], 'optionalAccess', _9 => _9.children, 'optionalAccess', _10 => _10.Gate]);
      if (gateCtx) {
    ...
    let orExpr = _optionalChain([ctx, 'optionalAccess', _11 => _11.orExpr, 'access', _12 => _12[0]]);
    if (decision === "reject") {
      if (!("elseExpr" in ctx))
        return "";
...
function getTokens(v2Compatible) {
  ...
  const Symbol3 = createToken({ name: "Symbol", pattern: new RegExp(`(${Escaped.DYNAMIC}|${Escaped.STATIC}[A-Za-z_0-9])[A-Za-z_0-9]*(\\(\\))?`) });
  ...
  const tokens = [Gate, Entity, Weight, ELSE, OC, CC, OR, EQ, Symbol3, DYN, STAT, AMP, Transform, OS, CS, PendingGate, Raw];
...
var _RiGrammar = class _RiGrammar {
  ...
  constructor(rules = {}, context = {}, options = {}) {
    this.rules = {};
    this.context = context;
    this.scripting = _nullishCoalesce(_optionalChain([options, 'access', _15 => _15.RiTa, 'optionalAccess', _16 => _16.riscript]), () => ( new RiScript(options)));
  ...
  toString(options = {}) {
    ...
    let lb = _optionalChain([options, 'optionalAccess', _17 => _17.linebreak]);
...
var RiQuery = class extends _mingo.Query {
  ...
  operands() {
    ...
    while (_optionalChain([stack, 'optionalAccess', _18 => _18.length]) > 0) {
...
var _RiScript = class _RiScript {
  ...
  _evaluate(options) {
    ...
    let last2, endingBreak = this.regex.EndingBreak.test(input);
...
Input:  '${escapeText2(input)}' ctx=${visitor.lookupsToString()}`);
    ...
    for (let i = 1; expr !== last2 && i <= 10; i++) {
      ...
      options.input = expr;
      expr = _nullishCoalesce(this.lexParseVisit(options), () => ( ""));
      if (trace) {
...
Final: '${result}'`);
  ...
  static pluralize(s, pluralizer) {
    if (!_optionalChain([pluralizer, 'optionalAccess', _19 => _19.pluralize])) {
  ...
  static articlize(s, phonemeAnalyzer) {
    ...
    if (!_optionalChain([phonemeAnalyzer, 'optionalAccess', _20 => _20.phones])) {
    ...
    let phones = phonemeAnalyzer.phones(first2, { silent: true });
    return (_optionalChain([phones, 'optionalAccess', _21 => _21.length]) && Vowels.test(phones[0]) ? "an " : "a ") + s;
  }
  /**
...
exports.RiGrammar = RiGrammar; exports.RiScript = RiScript;
